...
import { logLevel, logFileEnabled, keepLogsInProd, logDirectory, dateFormat, unixFormat, nodeEnv, keepLogsFor } from '../config/config.server';
...
if (process.env.LOG_TO_CONSOLE === 'true') {
  logger.add(new winston.transports.Console({
    level: logLevel || 'info', // Set default log level
...
if (logFileEnabled && !(nodeEnv == 'production' && keepLogsInProd === false)) {
  logger.add(new DailyRotateFile({
    ...
    datePattern: dateFormat, // Rotate log files daily
